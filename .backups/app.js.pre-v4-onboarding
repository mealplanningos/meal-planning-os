...
const CURRENT_UPDATE_VERSION = '2026-04-08-3';
...
function migrateSlotSchema(){
  try {
    ...
    // Carry legacy AM/PM notes to breakfast/dinner slot notes (lossless, additive).
    if(weekNotes && typeof weekNotes === 'object'){
      DAYS.forEach(d=>{
        const wn = weekNotes[d];
        if(!wn || typeof wn !== 'object') return;
        if(!mealPlan[d]) mealPlan[d] = {};
        if(wn.am && (!mealPlan[d].breakfast || !mealPlan[d].breakfast.note)){
          if(!mealPlan[d].breakfast) mealPlan[d].breakfast = {state:'skipped', recipeId:null, note:''};
          if(!mealPlan[d].breakfast.note) mealPlan[d].breakfast.note = String(wn.am);
          touched = true;
        }
        if(wn.pm && (!mealPlan[d].dinner || !mealPlan[d].dinner.note)){
          if(!mealPlan[d].dinner) mealPlan[d].dinner = {state:'skipped', recipeId:null, note:''};
          if(!mealPlan[d].dinner.note) mealPlan[d].dinner.note = String(wn.pm);
          touched = true;
        }
      });
    }
...
function openPlanWeekModal1(){
  // Seed the draft from current mealPlan so "Adjust" feels like editing, not restarting.
  _planDraft = { cook: {}, notes: {} };
  DAYS.forEach(d=>MEALS.forEach(m=>{
    const s = getSlot(d,m);
    const key = d+'_'+m;
    if(s && s.state==='cook') _planDraft.cook[key] = true;
    if(s && s.note) _planDraft.notes[key] = s.note;
  }));
  _renderPlanModal1();
...
function planModal1Next(){
  closePlanModal1();
  openPlanWeekModal2();
}

function openPlanWeekModal2(){
  _renderPlanModal2();
  const ov = document.getElementById('planModal2');
  if(ov) ov.classList.add('open');
}

// Per-meal rotating placeholder prompts — real schedule-conflict examples only
const _NOTE_PROMPTS = {
  breakfast: [
    'early meeting',
    'commute to office',
...
function _notePlaceholder(meal, idx){
  const arr = _NOTE_PROMPTS[meal] || ['add a note'];
  return 'e.g. ' + arr[idx % arr.length];
}

function _renderPlanModal2(){
  const body = document.getElementById('planModal2Body');
  if(!body) return;
  // Group by day: each day shows its cook slots (and optionally skipped slots).
  const perDay = {};
  let totalCook = 0, totalSkip = 0;
  DAYS.forEach(d=>{
    perDay[d] = { cook: [], skip: [] };
    MEALS.forEach(m=>{
      const key = d+'_'+m;
      if(_planDraft.cook[key]){ perDay[d].cook.push({meal:m,key}); totalCook++; }
      else { perDay[d].skip.push({meal:m,key}); totalSkip++; }
    });
  });

  let html = '';
  const _guideActive = !isOnboardingComplete() || _guideMode || !hasSeenGuide();
  if(_guideActive){
    html += '<div class="plan-modal-step-label">Step 2 of 4</div>';
  }
  html += '<div class="plan-modal-intro">Anything to plan around? <span style="color:var(--text-3);font-size:12px">(all optional)</span></div>';

  if(totalCook===0){
    html += '<div style="padding:16px;text-align:center;color:var(--text-3);font-size:13px">No meals selected to cook this week. That\'s fine — tap <strong>Done</strong> to head Home.</div>';
  } else {
    html += '<div class="plan-days-list">';
    let promptIdx = 0;
    DAYS.forEach(d=>{
      if(perDay[d].cook.length===0) return; // only days with cook slots
      const weekend = (d==='saturday'||d==='sunday');
      html += `<div class="plan-day-group${weekend?' weekend':''}">`;
      html += `<div class="plan-day-head">${DAY_FULL[d]}</div>`;
      html += '<div class="plan-day-meals">';
      perDay[d].cook.forEach(({meal,key})=>{
        const noteVal = _planDraft.notes[key] || '';
        const ph = _notePlaceholder(meal, promptIdx++);
        html += `<div class="plan-note-row"><div class="plan-note-label"><span class="plan-note-check">✓</span> ${MEAL_LABEL[meal]}</div><input type="text" class="plan-note-input" data-key="${key}" value="${_esc(noteVal)}" placeholder="${_esc(ph)}" oninput="_planDraft.notes['${key}']=this.value"></div>`;
      });
      html += '</div></div>';
    });
    html += '</div>';
...
function renderOnboardingUI(){
  ...
  // Toggle setup guide row inside Settings modal based on onboarding state
  const guideRow=document.getElementById('settingsGuideRow');
  if(guideRow){
    guideRow.style.display=isOnboardingComplete()?'':'none';
    const titleEl = guideRow.querySelector('.settings-row-title');
    if(titleEl) titleEl.textContent=(_guideMode||!hasSeenGuide())?'📖 Hide Setup Guide':'📖 Show Setup Guide';
  }

  const showGuide = !isOnboardingComplete() || _guideMode || !hasSeenGuide();

  // IDs that may hold onboarding content — cleared on every render
  ...
  // Reset before re-rendering so stale banners (e.g. Step 1 when we should now show Step 3) don't linger
  clearAllBanners();

  // Helper: build a banner with optional Next button on the right
  // phaseLabel = e.g. "Setup 1 of 3" or "Step 1 of 4"
  ...
  // ─── SETUP PHASE (1–3): one-time foundation ───
  // Setup 1 of 3 — Recipes
  ...
  if(b1) b1.innerHTML=obBanner('Setup 1 of 3','Build Your Recipe Library','Browse the recipe library or add your own meals — anything you already cook and enjoy. These are the building blocks for your entire plan. You only need to do this once; come back and add more anytime.', next1);

  // Setup 2 of 3 — No-Decision Menu
  const menuRecipes=recipes.filter(r=>r.onMenu);
  const next2=menuRecipes.length>=1?`<button class="btn btn-primary btn-next" onclick="switchTab('freezer')">Next →</button>`:'';
  const b2=document.getElementById('onboardingBanner-2');
  if(b2) b2.innerHTML=obBanner('Setup 2 of 3','Fill Your No-Decision Menu','Pick the meals you want on rotation this week from your recipe library. This becomes your short list — the only options you choose from when planning. Fewer choices means zero decision fatigue.', next2);

  // Setup 3 of 3 — Freezer (optional). Advances to Step 1 on Home, does NOT complete onboarding.
  const next6=`<button class="btn btn-primary btn-next" onclick="switchTab('dashboard')">Next →</button>`;
  const b6=document.getElementById('onboardingBanner-6');
  if(b6) b6.innerHTML=obBanner('Setup 3 of 3','Stock Your Freezer (Optional)','Already have meals in the freezer? Log them here so you can drop them into any day of your plan. Totally optional — skip if your freezer is empty and come back anytime you batch cook.', next6);

  // ─── WEEKLY FLOW PHASE (Steps 1–4): the repeating planning loop ───
  // Count cook slots and which ones have recipes assigned, to decide
  // whether the dashboard shows Step 1 (plan) or Step 3 (assign).
  let totalCookSlots = 0;
  let unassignedCookSlots = 0;
  DAYS.forEach(d=>MEALS.forEach(m=>{
    const s = mealPlan[d]&&mealPlan[d][m];
    if(s && s.state==='cook'){
      totalCookSlots++;
      if(!s.recipeId && !s.freezerId) unassignedCookSlots++;
    }
  }));
  // A week is considered "planned" if ANY slot entry exists (cook or skipped).
  // This matches how the Plan This Week modal writes state.
  let anySlotTouched = false;
  DAYS.forEach(d=>MEALS.forEach(m=>{
    if(mealPlan[d]&&mealPlan[d][m]) anySlotTouched = true;
  }));

  const bDash=document.getElementById('onboardingBanner-dash');
  if(bDash){
    if(!anySlotTouched){
      // Step 1 — plan this week
      const step1Btn=`<button class="btn btn-primary btn-next" onclick="openPlanWeekModal1()">Plan This Week →</button>`;
      bDash.innerHTML=obBanner('Step 1 of 4','Plan Your Week','Tap Plan This Week and pick which meals you\'re actually cooking. Takes about 30 seconds. Anything you don\'t pick, we\'ll treat as leftovers, eating out, or skipped — no pressure.', step1Btn);
    } else {
      // Step 3 — assign recipes to cook slots. Content varies by state.
      let title, desc, btn;
      if(totalCookSlots === 0){
        // Nothing to cook this week — subtle nudge to Groceries
        title = 'Nothing to Cook This Week';
        desc = 'You\'ve marked every meal as skipped, leftovers, or eating out. That\'s fine — head to Groceries for your flex & backup list just in case.';
        btn = `<button class="btn btn-primary btn-next" onclick="switchTab('grocerylist')">See Groceries →</button>`;
      } else if(unassignedCookSlots === 0){
        // All cook slots have recipes — move on
        title = 'All Meals Assigned';
        desc = 'Nice — every cooking slot has a recipe. Your grocery list is ready.';
        btn = `<button class="btn btn-primary btn-next" onclick="switchTab('grocerylist')">See Groceries →</button>`;
      } else {
        // Normal case — instruct user to tap slots
        title = 'Assign Your Meals';
        desc = `Tap any cooking slot below and pick a recipe from your no-decision menu. ${unassignedCookSlots} slot${unassignedCookSlots===1?'':'s'} still need${unassignedCookSlots===1?'s':''} a meal. <strong>Tip:</strong> set servings per meal — your grocery list scales automatically.`;
        btn = `<button class="btn btn-secondary btn-next" onclick="switchTab('grocerylist')">Skip to Groceries →</button>`;
      }
      bDash.innerHTML=obBanner('Step 3 of 4', title, desc, btn);
    }
  }

  // Step 4 — Groceries. "I'm Done" here is what flips firstRunComplete=true.
  const isReviewMode = _guideMode || (isOnboardingComplete() && !hasSeenGuide());
  const step4Btn = isReviewMode
  ...
  if(b5) b5.innerHTML=obBanner('Step 4 of 4','Grab Your Grocery List','Your list is built from the meals you assigned — quantities calculated, sorted by store section. Check items off as you shop, and use the Flex & Backup section for safety-net meals. When you\'re done, tap I\'m Done to retire this guide.', step4Btn);
}
...
// ╚═══════════════════════════════════════╝
const INFO_TIPS = {
  dashboard:   'Your week at a glance — auto-built from Steps 1-3 (Schedule → Meals → Groceries). Finish those steps first and your plan will show up here. Tap any recipe name to open cooking instructions.',
...
async function loadAndRender(userId) {
  ...
  if(savedTab && tabMigration[savedTab]) savedTab=tabMigration[savedTab];
  const validTabs = ['dashboard','recipes','menu','grocerylist','freezer'];
  // New users (onboarding not complete) start at Recipes; returning users default to Dashboard
  const defaultTab = isOnboardingComplete() ? 'dashboard' : 'recipes';
  switchTab(savedTab && validTabs.includes(savedTab) ? savedTab : defaultTab);
  renderAll();
  checkWhatsNew(cloud);
